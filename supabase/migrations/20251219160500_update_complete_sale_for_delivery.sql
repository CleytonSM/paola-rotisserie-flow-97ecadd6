...
BEGIN
    INSERT INTO public.sales (
        ...
        COALESCE(p_sale_data->>'order_status', 
                 CASE 
                    WHEN (p_sale_data->>'scheduled_pickup') IS NOT NULL THEN 'received' 
                    WHEN (p_sale_data->>'is_delivery')::BOOLEAN IS TRUE THEN 'received'
                    ELSE 'delivered' 
                 END),
        COALESCE((p_sale_data->>'is_delivery')::BOOLEAN, false),
        (p_sale_data->>'delivery_address_id')::UUID,
        COALESCE((p_sale_data->>'delivery_fee')::DECIMAL, 0)
